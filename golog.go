...
func (ml *MyLogger) Trace(v ...interface{}) {
	if ml.level >= LevelTrace {
		ml.log.Output(2, fmt.Sprintf(ml.name+"[Trace] %v", v...))
	}
}
...
func (ml *MyLogger) Tracef(format string, v ...interface{}) {
	if ml.level >= LevelTrace {
		ml.log.Output(2, fmt.Sprintf(ml.name+"[Trace] "+format, v...))
	}
}
...
func (ml *MyLogger) Panic(v ...interface{}) {
	s := fmt.Sprint(v...)
	if ml.level >= LevelCritical {
		ml.elog.Output(2, ml.name+"[Critical] "+s)
	}
	panic(s)
}
...
func Trace(v ...interface{}) {
	if std.level >= LevelTrace {
		std.log.Output(2, fmt.Sprintf("[Trace] %v", v...))
	}
}
...
func Tracef(format string, v ...interface{}) {
	if std.level >= LevelTrace {
		std.log.Output(2, fmt.Sprintf("[Trace] "+format, v...))
	}
}
